      ...
      <Sheet>
        ...
        <SheetContent side="top">
          ...
          {navItems.map((item , indx) => (
            <div key={indx}>
            <SheetClose asChild>
            <Link className="w-full flex justify-between items-center h-12  rounded-md px-3 my-2 text-lg border border-input bg-transparent shadow-sm hover:bg-accent hover:text-accent-foreground" to={item.to}>
                {item.name} {item.icon} 
            </Link>
          </SheetClose>
          </div>
